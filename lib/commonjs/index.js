...
const useRefs = () => {
  ...
  const tap = (0, _react.useRef)();
  const doubleTap = (0, _react.useRef)();
  const pinch = (0, _react.useRef)();
  return {
    pan,
    tap,
    doubleTap,
...
const ResizableImage = /*#__PURE__*/_react.default.memo(({
  ...
  const pinchActive = (0, _reactNativeReanimated.useSharedValue)(false);
  const panActive = (0, _reactNativeReanimated.useSharedValue)(false);
  ...
  const isActive = (0, _reactNativeReanimated.useDerivedValue)(() => currentIndex.value === index);
  ...
  const gestureHandler = (0, _reactNativeReanimated.useAnimatedGestureHandler)({
    ...
    onActive: ({
      ...
      const nextScale = (0, _utils.withRubberBandClamp)(s * ctx.scaleOffset, 0.55, maxScale, [1, maxScale]);
  ...
  const singleTapHandler = (0, _reactNativeReanimated.useAnimatedGestureHandler)({
    onActive: () => {
      if (onTap) {
  ...
  const doubleTapHandler = (0, _reactNativeReanimated.useAnimatedGestureHandler)({
    onActive: ({
      x,
      y,
      numberOfPointers
    ...
    onActive: ({
      ...
      if (!ctx.isVertical || scale.value > 1) {
        ...
        if (hideAdjacentImagesOnScaledImage && disableTransitionOnScaledImage) {
          ...
          const moveX = (0, _utils.withRubberBandClamp)(ctx.initialTranslateX + translationX - clampedX, 0.55, width, disabledTransition ? [getPosition(index), getPosition(index + 1)] : [getPosition(length - 1), 0]);
          ...
          if (loop) {
            ...
            translateX.value = (0, _utils.withRubberBandClamp)(ctx.initialTranslateX + translationX - clampedX, 0.55, width, disableTransitionOnScaledImage && scale.value > 1 ? [getPosition(index), getPosition(index + 1)] : [getPosition(length - 1), 0]);
          }
...
const GalleryComponent = ({
  ...
  (0, _reactNativeReanimated.useAnimatedReaction)(() => currentIndex.value, newIndex => (0, _reactNativeReanimated.runOnJS)(changeIndex)(newIndex), [currentIndex]);
  (0, _react.useEffect)(() => {
